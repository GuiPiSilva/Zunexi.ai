...
export function AppShell({ children }: { children: ReactNode }) {
  ...
  useEffect(
    () => subscribeNotifications(() => setNotifications(loadNotifications())),
    [],
  );
  ...
  useEffect(() => {
    ...
    return () =>
      window.removeEventListener("inlabs:credits-changed", updateCredits);
  }, []);

  useEffect(() => {
    function close(event: MouseEvent) {
      const target = event.target as Node;
      if (notificationBox.current && !notificationBox.current.contains(target))
        setNotificationOpen(false);
      if (profileBox.current && !profileBox.current.contains(target))
        setProfileOpen(false);
    }
  ...
  return (
    <div className="app-shell min-h-screen bg-background text-foreground lg:flex">
      ...
      <aside
        className={`app-sidebar fixed inset-y-0 left-0 z-50 flex w-[276px] flex-col overflow-hidden border-r border-sidebar-border bg-sidebar/95 shadow-2xl backdrop-blur-xl transition-transform lg:sticky lg:top-0 lg:h-dvh lg:w-[236px] xl:w-[252px] 2xl:w-[276px] ${open ? "translate-x-0" : "-translate-x-full lg:translate-x-0"}`}
      >
        <div className="app-sidebar-brand flex h-[76px] shrink-0 items-center border-b border-sidebar-border px-4">
          <img
            src={logoFull}
            alt="Zunexi.ai"
            className="brand-logo-full h-14 w-auto max-w-[225px] object-contain lg:max-w-[195px] xl:max-w-[210px] 2xl:max-w-[225px]"
          />
        </div>

        <div className="app-sidebar-nav min-h-0 flex-1 overflow-y-auto overscroll-contain px-4 pb-3 pt-5">
          <div className="app-sidebar-label mb-2 px-3 text-[10px] font-semibold uppercase tracking-[0.22em] text-muted-foreground">
            Central Zunexi
          </div>
          <nav className="space-y-1.5">
            {NAV.map((item) => {
              ...
              const locked = Boolean(
                item.feature &&
                credits &&
                !credits.features.includes(item.feature),
              );
              return (
                <Link
                  key={item.to}
                ...
                >
                  ...
                  {locked && (
                    <LockKeyhole className="h-3.5 w-3.5 text-muted-foreground/70" />
                  )}
                </Link>
              );
            })}
          ...
          <div className="app-sidebar-credits mb-3 flex items-center gap-2 rounded-xl border border-primary/20 bg-primary/8 px-3 py-2.5 text-xs">
            ...
            <div className="min-w-0">
              <div className="font-semibold">
                {credits?.unlimited
                  ? "Créditos infinitos"
                  : `${credits?.remaining ?? "—"} créditos no mês`}
              </div>
              <div className="mt-0.5 text-[10px] uppercase tracking-[.14em] text-muted-foreground">
                Plano {credits?.planName || "—"}
              </div>
            </div>
          </div>
          <div className="app-sidebar-profile flex items-center gap-3 rounded-xl border border-sidebar-border bg-card/55 p-3">
            <div className="brand-logo-tile grid h-10 w-10 shrink-0 place-items-center overflow-hidden rounded-xl border border-primary/25 p-1.5 shadow-[0_0_20px_rgba(139,92,246,0.16)]">
              <img
                src={logoIcon}
                alt="Zunexi.ai"
                className="brand-logo-icon h-full w-full object-contain"
              />
            </div>
            <div className="min-w-0 flex-1">
              <div className="truncate text-sm font-semibold" title={userName}>
                {userName}
              </div>
              <div className="truncate text-[11px] text-muted-foreground">
                Plano {credits?.planName || "carregando"}
              </div>
            </div>
            <button
              onClick={logout}
              title="Sair"
              className="rounded-lg p-2 text-muted-foreground transition hover:bg-secondary/70 hover:text-foreground"
            >
      ...
      <div className="app-main min-w-0 flex-1">
        <header className="app-topbar sticky top-0 z-30 flex h-[76px] items-center gap-3 border-b border-border/80 bg-background/75 px-4 backdrop-blur-2xl sm:px-6 lg:px-8">
          ...
          <div className="app-topbar-search ml-auto hidden min-w-0 w-full max-w-sm items-center gap-2 rounded-xl border border-border bg-card/70 px-3 py-2.5 min-[1180px]:flex">
            <Search className="h-4 w-4 shrink-0 text-muted-foreground" />
            <input
              aria-label="Buscar"
              placeholder="Buscar projetos e conteúdos..."
              className="min-w-0 w-full bg-transparent text-sm outline-none placeholder:text-muted-foreground"
            />
            <kbd className="shrink-0 rounded border border-border bg-secondary px-1.5 py-0.5 text-[10px] text-muted-foreground">
              ⌘K
            </kbd>
          </div>
          ...
          <div ref={notificationBox} className="relative">
            ...
            >
              ...
              {unread > 0 && (
                <span className="absolute -right-1 -top-1 grid min-h-4 min-w-4 place-items-center rounded-full bg-primary px-1 text-[9px] font-bold text-white">
                  {unread > 9 ? "9+" : unread}
                </span>
              )}
            </button>
                ...
                <div className="max-h-[420px] overflow-y-auto">
                  {notifications.length === 0 ? (
                    <div className="grid min-h-36 place-items-center px-6 text-center text-sm text-muted-foreground">
                      As conclusões das suas criações aparecerão aqui.
                    </div>
                  ) : (
                    notifications.map((item) => (
                      <button
                        ...
                        className="block w-full border-b border-border/70 px-4 py-3 text-left transition hover:bg-secondary/60"
                      >
                        <div className="flex items-start gap-3">
                          <span
                            className={`mt-1 h-2 w-2 shrink-0 rounded-full ${item.kind === "error" ? "bg-red-400" : item.kind === "success" ? "bg-emerald-400" : "bg-primary"}`}
                          />
                          <div className="min-w-0">
          ...
          <div ref={profileBox} className="relative hidden sm:block">
            ...
            >
              <div className="brand-logo-tile grid h-8 w-8 place-items-center overflow-hidden rounded-lg border border-primary/25 p-1">
                <img
                  src={logoIcon}
                  alt="Zunexi.ai"
                  className="brand-logo-icon h-full w-full object-contain"
              ...
              <ChevronDown
                className={`hidden h-3.5 w-3.5 text-muted-foreground transition-transform xl:block ${profileOpen ? "rotate-180" : ""}`}
              />
            </button>
            ...
            {profileOpen && (
              <div
                role="menu"
                className="absolute right-0 top-12 z-50 w-60 overflow-hidden rounded-2xl border border-border bg-popover p-2 shadow-2xl"
              >
