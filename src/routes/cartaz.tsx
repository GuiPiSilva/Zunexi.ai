...
function NovoCartaz() {
  ...
  useEffect(() => {
    ...
    function syncFromJob() {
      ...
      const savedForm = latest.payload.form as Partial<typeof form> | undefined;
  ...
  async function executeCartazJob(initialJob: CreationJob, memoryPhoto?: string) {
    ...
    const started = await withCreationJobLock(initialJob.id, async () => {
      ...
      try {
        ...
        toast.success("Cartaz finalizado com texto e salvo em Meus projetos.");
    ...
    <AppShell>
      <div className="page-wrap space-y-7">
        ...
        <div className="panel grid gap-3 p-4 sm:grid-cols-3 sm:p-5">
          {[{ n: 1, title: "Informações", sub: "Dados do evento" }, { n: 2, title: "Geração", sub: "Texto e imagem com IA" }, { n: 3, title: "Final", sub: "Visualize e exporte" }].map((item, index) => (
